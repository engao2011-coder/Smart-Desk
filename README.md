...
| **Signal** | A mechanical, **DCA-oriented** rating computed on-device from trend (price vs a moving average of weekly closes), momentum, and position in the 52-week range (double-weighted to favour buying dips). Buy-biased — it never recommends a full sell; ratings are **STRONG BUY / BUY / HOLD / TRIM**. Shown as a colour-coded pill on the Home markets card and a compact letter tag (SB/B/H/T) per row on the Stocks page. Every threshold (average length, trend/momentum bands, range edge, minimum history) is tunable **per stock** from the web settings page → **Buy/Sell Signal**. **Not financial advice** — it's a slow, weekly-horizon technical indicator, not a trading cue |
