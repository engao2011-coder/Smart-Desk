/*
 * ui.h — TFT Display UI (full-screen page architecture)
 *
 * Colour palette, layout constants, and all drawing/touch routines for the
 * 240×320 portrait ILI9341 TFT on the ESP32-2432S028R ("Cheap Yellow Display").
 *
 * ── Page architecture ───────────────────────────────────────────────────────
 * Every page fills the entire 240×320 screen (no persistent status bar or hero).
 *
 *  PAGE_HOME (0) — dashboard tile grid
 *  ┌──────────────────────────┐  y = 0
 *  │  Hero         (120 px)   │  Large clock + date (left) | Weather chip (right)
 *  ├──────────────────────────┤  y = 120
 *  │  Prayer band  (108 px)   │  Next prayer name, time, and countdown
 *  ├──────────────────────────┤  y = 228
 *  │  Markets band  (92 px)   │  Top-moving stock: name, price, daily change
 *  └──────────────────────────┘  y = 320
 *
 ...
 *  • Tap the Prayer band                        → opens Prayer
 *  • Tap the Markets band                       → opens Stocks
 *  • Tap anywhere on a detail page              → returns to Home
 *  • 60 s without touch                         → returns to Home automatically
 *  Pages do not auto-rotate; they only change on touch or prayer/break events.
 *
 * ── Fonts ───────────────────────────────────────────────────────────────────
 *  Custom Montserrat Bold GFX fonts (Adafruit format, generated by tools/genfont.py):
 *    MontserratBold24pt7b — large clock (HH:MM)
 *    MontserratBold18pt7b — Azan fullscreen prayer name
 *    MontserratBold14pt7b — section headings, weather temperature
 *    MontserratBold12pt7b — weather detail, prayer times
 *    MontserratBold9pt7b  — banner text, stock names
 *    MontserratSB6pt7b    — small supplementary labels
 *  Default font (nullptr / size 1 = 6 px native) — all other small text.
 */
