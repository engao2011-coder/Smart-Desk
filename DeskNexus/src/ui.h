...
// Status bar (y=0..23)
// ---------------------------------------------------------------------------
static void drawStatusBar(bool wifiOk, const String& dateStr) {
    ...
    int tw = tft.textWidth(dateStr);
    tft.setCursor((SCREEN_W - tw) / 2, 8);
    tft.print(dateStr);

    // IP address (right-aligned, only when in AP mode)
    if (!wifiOk) {
        String apLabel = "AP mode";
        tft.setTextColor(C_GOLD, C_BG);
        int aw = tft.textWidth(apLabel);
        tft.setCursor(SCREEN_W - aw - 4, 8);
        tft.print(apLabel);
    }
...
// Full screen redraw
// ---------------------------------------------------------------------------
static void redraw(bool wifiOk, const String& ip,
                   const struct tm& t, const String& dateStr) {
    drawStatusBar(wifiOk, dateStr);
    drawClock(t);
    drawWeather();
    drawTabs();
